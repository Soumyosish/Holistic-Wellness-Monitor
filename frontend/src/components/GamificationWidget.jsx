...
function GamificationWidget() {
  ...
  return (
    <div className="relative overflow-hidden bg-white rounded-[2.5rem] p-6 shadow-xl shadow-slate-200/50 border border-slate-100 h-full flex flex-col group transition-all duration-500 hover:shadow-2xl hover:shadow-amber-100/40">
      ...
      <div className="absolute -bottom-5 -left-5 w-32 h-32 bg-yellow-100/30 rounded-full blur-2xl pointer-events-none"></div>

      {/* Header */}
      <div className="flex items-center justify-between z-10 mb-6">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-linear-to-br from-amber-300 to-yellow-500 rounded-2xl shadow-lg shadow-amber-200 text-white transform group-hover:rotate-12 transition-transform duration-300">
            <Trophy
              className="w-5 h-5"
              fill="currentColor"
              fillOpacity={0.2}
          ...
          <div>
            ...
            <div className="text-xs font-semibold text-amber-500 flex items-center gap-1">
              Level {nextLevel - 1} <span className="text-slate-300">•</span>{" "}
              {data.totalPoints} XP
            </div>
          </div>
        </div>
      ...
      <div className="mb-6 z-10">
        <div className="flex justify-between text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-wider">
          ...
          <span>
            {Math.round(progress)}% to Lvl {nextLevel}
          </span>
        </div>
        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-linear-to-r from-amber-400 to-yellow-300 rounded-full shadow-lg shadow-amber-200"
      ...
      <div className="grid grid-cols-4 gap-3 z-10">
        {unlockedBadges.slice(0, 4).map((badge) => (
          <div
            key={badge.id}
            className="flex flex-col items-center group/badge"
          >
            <div className="relative mb-2">
              <div className="w-12 h-12 rounded-2xl bg-linear-to-br from-slate-50 to-slate-100 border border-slate-200 flex items-center justify-center text-2xl shadow-sm transition-all duration-300 group-hover/badge:scale-110 group-hover/badge:shadow-md group-hover/badge:border-amber-200">
                {badge.icon}
              </div>
              {/* Shine effect */}
