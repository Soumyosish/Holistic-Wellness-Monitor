...
import { Utensils, Plus, ChevronRight, Coffee, Sun, Moon, Cookie, Flame } from "lucide-react";
...
function TodaysMeals() {
  ...
  return (
    <div className="relative overflow-hidden bg-white rounded-[2.5rem] p-6 shadow-xl shadow-slate-200/50 border border-slate-100 h-full flex flex-col transition-all duration-500 hover:shadow-2xl hover:shadow-orange-100/40">
      ...
      <div className="absolute top-[-40px] right-[-40px] w-48 h-48 bg-orange-100/40 rounded-full blur-[60px] pointer-events-none"></div>
      <div className="absolute bottom-[20%] left-[-20px] w-32 h-32 bg-emerald-100/30 rounded-full blur-[40px] pointer-events-none"></div>

      {/* Header */}
      <div className="flex items-center justify-between z-10 mb-6">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-gradient-to-br from-orange-400 to-amber-500 rounded-2xl shadow-lg shadow-orange-200 text-white">
            <Utensils className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-slate-800 leading-tight">Nutrition</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-6 z-10 flex-1">
         ...
         <div className="flex-1 flex flex-col items-center justify-center relative">
            <div className="w-40 h-40 relative">
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-2xl font-black text-slate-800">{Math.round(totals.calories)}</span>
            ...
            <div className="flex gap-4 mt-2">
               <div className="text-center">
                  ...
                  <div className="text-xs font-bold text-slate-700">{Math.round(totals.protein)}g</div>
                  <div className="text-[10px] text-slate-400">Pro</div>
               </div>
               <div className="text-center">
                  <div className="w-2 h-2 rounded-full bg-amber-500 mx-auto mb-1"></div>
                  <div className="text-xs font-bold text-slate-700">{Math.round(totals.carbs)}g</div>
                  <div className="text-[10px] text-slate-400">Carb</div>
               </div>
               <div className="text-center">
                  <div className="w-2 h-2 rounded-full bg-rose-500 mx-auto mb-1"></div>
                  <div className="text-xs font-bold text-slate-700">{Math.round(totals.fats)}g</div>
         ...
         <div className="flex-1 flex flex-col gap-3 min-w-[200px]">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Recent</div>
            ...
            {meals.length > 0 ? (
               <div className="flex flex-col gap-3">
                  {meals.slice(0, 3).map((meal) => {
                     ...
                     return (
                        <div key={meal._id} className="flex items-center gap-3 p-2.5 rounded-2xl bg-slate-50/80 border border-slate-100 hover:bg-white hover:shadow-md hover:border-orange-100 transition-all group cursor-pointer" onClick={() => navigate("/meals")}>
                           <div className="p-2 bg-white rounded-xl shadow-sm text-slate-400 group-hover:text-orange-500 transition-colors">
                              <Icon size={16} />
                           </div>
                           <div className="flex-1 min-w-0">
                              <div className="text-sm font-bold text-slate-700 truncate">{meal.name}</div>
                              <div className="text-[10px] font-medium text-slate-400">{Math.round(meal.calories)} kcal</div>
                           </div>
                        </div>
                     );
                  })}
                  {meals.length < 3 && (
                     <button onClick={() => navigate("/meals")} className="flex items-center gap-2 p-2.5 rounded-2xl border border-dashed border-slate-200 text-slate-400 hover:text-orange-500 hover:border-orange-200 hover:bg-orange-50 transition-all justify-center text-xs font-bold">
               ...
               <div className="flex-1 flex flex-col items-center justify-center text-center p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  ...
                  <p className="text-xs font-medium text-slate-400 mb-3">No meals logged today</p>
                  <button 
                     onClick={() => navigate("/meals")}
