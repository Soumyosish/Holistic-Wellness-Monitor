...
import { Dumbbell, Plus, Search, Calendar, Trash2, Clock, Flame, ChevronRight } from "lucide-react";
...
function WorkoutTracker() {
  ...
  const handleCreateWorkout = async (e) => {
    ...
    try {
      ...
      fetchWorkouts();
      setNewWorkout({ name: "", duration: "", intensity: "medium", type: "strength", exercises: [] });
    } catch (error) {
  ...
  if (loading) return <div className="p-8 text-center">Loading workouts...</div>;
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          ...
          className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center gap-2"
        >
      ...
      <div className="space-y-4">
        {workouts.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-3xl shadow-sm border border-gray-100">
            ...
            <h3 className="text-xl font-semibold text-gray-800">No workouts logged</h3>
            <p className="text-gray-500 mt-2">Start tracking to see your progress!</p>
          </div>
        ) : (
          workouts.map((workout) => (
            <div key={workout._id} className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow p-6 border border-gray-100">
              <div className="flex justify-between items-start">
                <div className="flex items-start gap-4">
                  <div className={`p-4 rounded-xl ${
                    workout.type === 'cardio' ? 'bg-orange-50 text-orange-600' :
                    workout.type === 'strength' ? 'bg-blue-50 text-blue-600' :
                    'bg-purple-50 text-purple-600'
                  }`}>
                    {workout.type === 'cardio' ? <Flame className="w-6 h-6" /> : <Dumbbell className="w-6 h-6" />}
                  </div>
                  <div>
