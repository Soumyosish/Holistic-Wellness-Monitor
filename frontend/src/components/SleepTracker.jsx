...
import { Moon, Star, TrendingUp, CloudMoon, Clock, ChevronRight } from "lucide-react";
...
function SleepTracker() {
  ...
  const score = sleepData.sleepHours >= 7 ? "Optimal" : sleepData.sleepHours >= 6 ? "Good" : "Low";
      ...
      <div className="flex items-center justify-between z-10 mb-8">
        <div>
           <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-xl font-bold text-slate-800 tracking-tight">Sleep Health</h2>
           </div>
           <p className="text-sm font-medium text-slate-400 pl-1">Rest & Recovery</p>
        </div>
        ...
        <div className="text-right">
             ...
             <div className={`text-xs font-bold px-3 py-1 rounded-full inline-block ${
                 sleepData.sleepHours >= 7 ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'
             }`}>
               {score} Rest
             </div>
        </div>
         ...
         <div className="flex items-center gap-4">
            <div className="relative w-24 group/input">
               <input 
                  ...
                  onBlur={(e) => updateSleep(parseFloat(e.target.value) || 0, sleepData.sleepQuality || 3)}
               />
            </div>
               ...
               {[1, 2, 3, 4, 5].map((star) => (
                 <button
                   ...
                   className={`transition-all duration-300 hover:scale-125 hover:-translate-y-1 p-1 ${
                     star <= (sleepData.sleepQuality || 0) ? "text-amber-400 drop-shadow-sm" : "text-slate-200 hover:text-amber-200"
                   }`}
                 >
                   <Star size={20} fill={star <= (sleepData.sleepQuality || 0) ? "currentColor" : "currentColor"} />
                 </button>
               ))}
            </div>
         </div>
