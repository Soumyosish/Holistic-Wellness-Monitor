...
import { User, Activity, Target, Utensils, Award, Calculator, ChevronRight, Apple, Save, Droplet, Footprints, Flame } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">
            Personalized insights and daily targets to help you reach your peak performance.
          </p>
        </div>
              ...
              {[1, 2, 3].map((step) => (
                <div
                  ...
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${
                    currentStep >= step ? "bg-indigo-600 scale-125" : "bg-slate-300"
                  }`}
                />
              ))}
            </div>
                ...
                {currentStep === 1 && (
                  <div className="space-y-8 animate-fade-in-up">
                    ...
                    <div className="flex justify-end pt-6">
                      <button type="button" onClick={nextStep} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-colors shadow-lg shadow-indigo-200">
                        Next Step
                      </button>
                    </div>
                  </div>
                ...
                {currentStep === 2 && (
                  <div className="space-y-8 animate-fade-in-up">
                     <div className="flex items-center gap-4 mb-8">
                       ...
                       <h2 className="text-2xl font-bold text-slate-800">Activity & Goals</h2>
                    </div>
                    ...
                    <div className="space-y-6">
                       <div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {Object.entries(ACTIVITY_LEVELS).map(([key, { label, description }]) => (
                               <label key={key} className={`cursor-pointer p-4 rounded-xl border-2 transition-all ${formData.activityLevel === key ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-100 hover:border-slate-200'}`}>
                                  <input type="radio" name="activityLevel" value={key} checked={formData.activityLevel === key} onChange={handleInputChange} className="hidden" />
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {Object.entries(GOALS).map(([key, { label, icon: Icon }]) => (
                               <label key={key} className={`flex items-center gap-3 cursor-pointer p-4 rounded-xl border-2 transition-all ${formData.goal === key ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-100 hover:border-slate-200'}`}>
                                  <input type="radio" name="goal" value={key} checked={formData.goal === key} onChange={handleInputChange} className="hidden" />
                                  <Icon className={formData.goal === key ? 'text-emerald-500' : 'text-slate-400'} size={20} />
                                  <div className="font-semibold text-slate-800">{label}</div>
                               </label>
                            ))}
                          </div>
                     ...
                     <div className="flex justify-between pt-6">
                      <button type="button" onClick={prevStep} className="px-6 py-3 text-slate-500 font-semibold hover:text-slate-700">Back</button>
                      <button type="button" onClick={nextStep} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-colors shadow-lg shadow-indigo-200">
                        Next Step
                      </button>
                    </div>
                ...
                {currentStep === 3 && (
                  <div className="space-y-8 animate-fade-in-up">
                     <div className="flex items-center gap-4 mb-8">
                       ...
                       <h2 className="text-2xl font-bold text-slate-800">Diet Preferences</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                     ...
                     <div className="flex justify-between pt-6">
                      <button type="button" onClick={prevStep} className="px-6 py-3 text-slate-500 font-semibold hover:text-slate-700">Back</button>
                      <button type="submit" disabled={loading} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-colors shadow-lg shadow-indigo-200">
            ...
            <div className="space-y-8 animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                 ...
                 <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center justify-center relative overflow-hidden">
                    <h3 className="text-slate-500 font-semibold mb-2 uppercase tracking-wider text-xs z-10">Body Mass Index</h3>
                     <div className="h-[220px] w-full flex justify-center items-center z-10 relative">
                        <ResponsiveContainer width="100%" height="100%">
                          <RadialBarChart 
                            innerRadius="70%" 
                            outerRadius="100%" 
                            data={[{ name: 'BMI', value: calculatedMetrics.bmi, fill: getBMICategory(calculatedMetrics.bmi).color.includes('green') ? '#10b981' : getBMICategory(calculatedMetrics.bmi).color.includes('yellow') ? '#f59e0b' : '#ef4444' }]} 
                            startAngle={180} 
                            endAngle={0}
                            cy="70%"
                          >
                        ...
                        <div className="absolute top-[60%] left-1/2 transform -translate-x-1/2 -translate-y-[50%] text-center">
                           <span className="text-5xl font-extrabold text-slate-800 tracking-tighter">{calculatedMetrics.bmi}</span>
                           <p className={`text-sm font-bold mt-1 uppercase tracking-wide ${getBMICategory(calculatedMetrics.bmi).color}`}>{getBMICategory(calculatedMetrics.bmi).text}</p>
                        </div>
                     </div>
                 </div>
                 ...
                 <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center justify-center lg:col-span-1">
                    <h3 className="text-slate-500 font-semibold mb-2 uppercase tracking-wider text-xs">Macro Distribution</h3>
                     <div className="h-[220px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                           <PieChart>
                              <Pie
                                data={[
                                  { name: 'Protein', value: calculatedMetrics.dailyProteinTarget, unit: 'g', color: '#10b981' },
                                  { name: 'Carbs', value: calculatedMetrics.dailyCarbsTarget, unit: 'g', color: '#f59e0b' },
                                  { name: 'Fats', value: calculatedMetrics.dailyFatsTarget, unit: 'g', color: '#6366f1' },
                                ]}
                                cx="50%"
                                cy="50%"
                                innerRadius={60}
                                ...
                                dataKey="value"
                              >
                                {[{color: '#10b981'}, {color: '#f59e0b'}, {color: '#6366f1'}].map((entry, index) => (
                 ...
                 <div className="grid grid-cols-1 gap-4">
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between hover:shadow-md transition-shadow">
                       <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">BMR</p>
                          <p className="text-4xl font-extrabold text-slate-800">{calculatedMetrics.bmr}</p>
                          <p className="text-sm text-slate-500 font-medium">kcal / day</p>
                       </div>
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between hover:shadow-md transition-shadow">
                       <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Ideal Weight</p>
                          <p className="text-4xl font-extrabold text-slate-800">{calculatedMetrics.idealWeight}</p>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                 ...
                 <div className="bg-orange-500 rounded-3xl p-6 text-white shadow-xl shadow-orange-500/20 transform hover:-translate-y-1 transition-transform">
                    <div className="flex justify-between items-start mb-6">
                       <span className="p-3 bg-white/20 rounded-2xl"><Flame size={24} /></span>
                       <span className="text-xs font-extrabold bg-white/20 px-3 py-1 rounded-full uppercase tracking-wide">Target</span>
                    </div>
                    <div className="text-4xl font-extrabold mb-1">{calculatedMetrics.dailyCalorieTarget}</div>
                    <div className="text-sm font-medium opacity-90">Calories / day</div>
                 </div>
                 ...
                 <div className="bg-cyan-500 rounded-3xl p-6 text-white shadow-xl shadow-cyan-500/20 transform hover:-translate-y-1 transition-transform">
                    <div className="flex justify-between items-start mb-6">
                       <span className="p-3 bg-white/20 rounded-2xl"><Droplet size={24} /></span>
                       <span className="text-xs font-extrabold bg-white/20 px-3 py-1 rounded-full uppercase tracking-wide">Goal</span>
                    </div>
                    <div className="text-4xl font-extrabold mb-1">{(calculatedMetrics.dailyWaterGoal / 1000).toFixed(1)}</div>
                    <div className="text-sm font-medium opacity-90">Liters / day</div>
                 </div>
                 ...
                 <div className="bg-emerald-500 rounded-3xl p-6 text-white shadow-xl shadow-emerald-500/20 transform hover:-translate-y-1 transition-transform">
                    <div className="flex justify-between items-start mb-6">
                       <span className="p-3 bg-white/20 rounded-2xl"><Footprints size={24} /></span>
                       <span className="text-xs font-extrabold bg-white/20 px-3 py-1 rounded-full uppercase tracking-wide">Active</span>
                    </div>
                    <div className="text-4xl font-extrabold mb-1">{(calculatedMetrics.dailyStepGoal / 1000).toFixed(0)}k</div>
                    <div className="text-sm font-medium opacity-90">Steps / day</div>
                 </div>
                 ...
                 <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col justify-center gap-4">
                    <div className="flex items-center justify-between">
                       <span className="flex items-center gap-2 text-sm font-bold text-slate-600"><div className="w-2 h-2 rounded-full bg-emerald-500"></div> Protein</span>
                       <span className="font-bold text-slate-800">{calculatedMetrics.dailyProteinTarget}g</span>
                    </div>
                    <div className="flex items-center justify-between">
                       <span className="flex items-center gap-2 text-sm font-bold text-slate-600"><div className="w-2 h-2 rounded-full bg-amber-500"></div> Carbs</span>
                       <span className="font-bold text-slate-800">{calculatedMetrics.dailyCarbsTarget}g</span>
                    </div>
                    <div className="flex items-center justify-between">
                       <span className="flex items-center gap-2 text-sm font-bold text-slate-600"><div className="w-2 h-2 rounded-full bg-indigo-500"></div> Fats</span>
                       <span className="font-bold text-slate-800">{calculatedMetrics.dailyFatsTarget}g</span>
                    </div>
                 </div>
              </div>
